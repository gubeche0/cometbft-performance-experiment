...
df = pd.read_csv(file_path, sep=';', names=[
    'node', 'type', 'status', 'initialHeight', 'finalHeight', 'elapsedTime', 'startTime', 'endTime'
])
...
# df["latencia"] = (df['DataBloco'] - df['DataBloco'].shift(1)).dt.total_seconds()
# df.dropna(subset=['latencia'], inplace = True)

data = df["elapsedTime"].dt.total_seconds()
# data = df['elapsedTime'][0].microseconds
# data = df['elapsedTime'][0].microseconds
# data = df['elapsedTime'][0].microseconds
...
plt.xlabel('Transaction latency (s)', fontsize=20)
plt.ylabel('')
plt.ylim(0, 1)
plt.grid(True)
plt.savefig(f'transactions-latency-{file}.png')
plt.show()